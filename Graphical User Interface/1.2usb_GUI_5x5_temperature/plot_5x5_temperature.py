...
class PLOT_3D(QtWidgets.QWidget, Ui_Form):
    ...
    def save_data_init(self, filenname):
        data_head = ['Sensor' + str(i) for i in range(1, 33)]
        headers =   data_head

        self.init_savetime = time.time()
    ...
    def save_data(self):
        ...
        self.save_path = './lib/data_save/' + int_time + '_sensorT5X5.csv'

        if(button_state == 'Save data'):
            self.save_data_init(self.save_path)
    ...
    def save_timer(self):
        ...
        self.textBrowser_2.append("正在测检测——" + str(buf_len))  # 在指定的区域显示提示信息
    ...
    def update_plot(self):
        ...
        self.image = self.usbdata.image_construct.get(True)
        ...
        #显示第一个数值
        #self.textBrowser.append(str(self.get_z[0,6]))  # 在指定的区域显示提示信息
