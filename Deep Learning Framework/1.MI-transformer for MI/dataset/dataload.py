...
class MyDataset(Dataset):  #Dataset
    def __init__(self, path_dir = '../Processed data/', transform=None, time_lenth = 1, k_num = [1], noise_mean = 0,noise_std_max = 1):
        ...
        for participants_name in k_num:  #data
            ...
            filename_list = os.listdir(file_dir)  # numpydata path list
            filename_list.sort()  # 排序


            for filename in filename_list:

                data_dir = file_dir + '/' + str(filename)  # sensor data patch
                ...
                for i in range(len(readnumpy)-self.time_lenth): #时间序列分割
                    Label_data = readnumpy[i+time_lenth, 772 -3: 772]
                    Sensor_data = readnumpy[i:i+time_lenth, 1: 769]
